...
// Last update Wed Aug  2 21:43:28 2017 Gaëtan Léandre
//
...
function updateBusiness(businessId, businessName, game) {
    card.findOneAndUpdate({yelpId: businessId},
        {
            $set: {
                name: businessName,
                yelpId: businessId
        ...
        function (err, cards) {
            if (!err) {
                cardGame.findOneAndUpdate({game: game._id, card: cards._id}, {$set: {game: game._id, card: cards._id}}, {upsert: true, 'new': true}, function(){});
            }
        }
    );
}
